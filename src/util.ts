...
export const embed = (title: string, desc = "", color = COLORS.info) =>
  new EmbedBuilder().setTitle(title).setDescription(desc && desc.length > 0 ? desc : null).setColor(color);

export const isAdmin = (m?: GuildMember | null) =>
  !!m && (m.permissions.has("Administrator") || m.permissions.has("ManageGuild") || m.permissions.has("ManageMessages"));

export const safeIconName = (boss: string) => normalize(boss).replace(/[^a-z0-9]+/g, "_") || "unknown";
export const bossIconPath = (boss: string) => {
  ...
  try {
    if (fs.existsSync(ICONS_DIR)) {
        ...
        if (fs.existsSync(aliasPath)) {
          ...
          for (const [k, v] of Object.entries(raw)) {
            if (typeof v === "string" && normalize(String(v)) === base) {
              ...
              if (!tokens.includes(ak)) tokens.unshift(ak);
            }
          }
        }
      } catch {
      ...
      for (const t of tokens) {
        ...
        if (exact) return path.join(ICONS_DIR, exact);

        // Filename that contains the token
        const incl = files.find((f) => f.includes(t));
        if (incl) return path.join(ICONS_DIR, incl);
      }
    }
  } catch {
...
export const fileFromBuffer = (buf: Buffer, name: string) => new AttachmentBuilder(buf, { name });
