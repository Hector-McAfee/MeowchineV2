...
import {
  suggestTileNumbers,
  suggestEmptyTileNumbers,
  suggestBossesForSetup,
  suggestAllCatalogBosses,
  suggestDropsForBoss,
  suggestSubmitDropsForBoss,
  suggestUsedBosses,
  suggestChestTargets,
} from "./data.js";
...
async function safeRespond(i: AutocompleteInteraction, choices: any[]) {
  if ((i as any).responded) return;
...
let BOSS_CATALOG: Record<string, string[]> = {};
try {
...
export async function handleAutocomplete(i: AutocompleteInteraction) {
  try {
    ...
    // --- Setup commands ---
    if (name === "add_boss") {
      if (focused.name === "tile") return safeRespond(i, await suggestEmptyTileNumbers(i));
      if (focused.name === "boss") return safeRespond(i, await suggestAllCatalogBosses(i, BOSS_CATALOG));
    }

    if (name === "add_drop") {
      if (focused.name === "boss") return safeRespond(i, await suggestBossesForSetup(i));
      if (focused.name === "drop") return safeRespond(i, await suggestDropsForBoss(i, BOSS_CATALOG));
      if (focused.name === "amount") return safeRespond(i, [{ name: "1", value: 1 }, { name: "2", value: 2 }, { name: "3", value: 3 }]);
    }

    if (name === "set_teams_drop") {
      if (focused.name === "boss") return safeRespond(i, await suggestUsedBosses(i));
      if (focused.name === "drop") return safeRespond(i, await suggestSubmitDropsForBoss(i));
    }

    if (name === "remove_boss") {
      if (focused.name === "tile") return safeRespond(i, await suggestTileNumbers(i));
    }
