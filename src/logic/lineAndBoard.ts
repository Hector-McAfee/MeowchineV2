...
export function checkAnyLineComplete(state: EventState, team: string): boolean {
  ...
  for (let r=0; r<n; r++) {
    ...
    for (let c=0; c<n; c++) {
      const idx = r*n + c + 1;
      if (idx > cap || !tileComplete(state, team, idx)) { ok = false; break; }
    }
    if (ok) return true;
  }
  for (let c=0; c<n; c++) {
  ...
  for (let i=0; i<n; i++) {
    ...
    if (idx1 > cap || !tileComplete(state, team, idx1)) okD1 = false;
    if (idx2 > cap || !tileComplete(state, team, idx2)) okD2 = false;
  }
  return okD1 || okD2;
}

export async function onBoardCompleteMaybeAnnounce(guild: Guild, state: EventState, team: string) {
  const n = parseInt(state.options.boardSize[0]);
  const cap = maxTiles(state.options.boardSize);
  let all = true;
  for (let i=1; i<=cap; i++) {
    if (!tileComplete(state, team, i)) { all = false; break; }
  }
  if (!all) return;
  ...
  if (!state.placements.includes(team)) {
    ...
    if (state.announcementsChannelId) {
      const ann = guild.channels.cache.get(state.announcementsChannelId) as TextChannel | undefined;
      ...
      await ann?.send({
        embeds: [embed("Board Complete 🏁",
          `Team **${team}** has completed the board!\n` +
