...
export async function execute(i: ChatInputCommandInteraction) {
  ...
  for (const item of pendingDrops) {
    try {
      ...
      const baseEmbed = oldMessage?.embeds?.[0]
        ...
        : embed(
            "Drop Submission",
            `**Team ${item.pending.team}** submitted **${item.pending.drop}** from **${item.pending.boss}** *(pending)*`
          );
  ...
  return i.reply({
    embeds: [
      embed(
        "Pending Drops Reposted",
        `Reposted **${reposted}** pending drop(s) in <#${outCh.id}>.` +
