...
export async function execute(i: ChatInputCommandInteraction) {
  ...
  const allow = s.options.allowPlayersCheckTeams || isAdmin(i.member as any);
  if (!allow) return i.reply({ embeds: [embed("Forbidden","Players checking other teams is disabled.")], ephemeral: true });

  const team = i.options.getInteger("team", true).toString();
  const img = await renderBoardImage(s as any, team);
  ...
  const lines = pending.map(p => `• Tile ${(p as any).tile} — **${(p as any).drop}** *(pending)*`);
