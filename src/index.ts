...
import { Client, GatewayIntentBits, Partials, Events, ChatInputCommandInteraction, Interaction } from "discord.js";
...
const registrar = new Registrar(rest as any, appId);
...
client.once(Events.ClientReady, async c => {
  ...
  for (const [gid] of client.guilds.cache) {
    ...
    await registrar.setGuildCommands(gid, body as any).catch(console.error);
  }
...
client.on(Events.GuildCreate, async guild => {
  ...
  const body = s.active ? full : minimal;
  await registrar.setGuildCommands(guild.id, body as any).catch(console.error);
});
...
client.on(Events.InteractionCreate, async (interaction) => {
  ...
  const name = interaction.commandName as keyof typeof handlers;
  ...
  try {
    await fn(interaction as ChatInputCommandInteraction);
  } catch (e) {
    console.error(e);
