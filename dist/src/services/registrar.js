...
function looksLike(name, prefix) {
    return name.toLowerCase().startsWith(prefix);
}
function shouldAutocomplete(name) {
    const n = name.toLowerCase();
    return (looksLike(n, "boss") ||
        looksLike(n, "drop") ||
        looksLike(n, "team") ||
        looksLike(n, "tile") ||
        n === "position" ||
...
function sortOptionsRequiredFirst(options) {
    ...
    withIndex.sort((a, b) => {
        ...
        if (ar !== br)
            return br - ar; // required before optional
        return a.idx - b.idx;
    });
...
function ensureAutocompleteOnOptions(options) {
    if (!Array.isArray(options))
        return;
    for (const o of options) {
        if (o?.name && shouldAutocomplete(o.name)) {
            ...
            if (Array.isArray(o.choices))
                delete o.choices; // choices conflict with autocomplete
        }
...
export function normalizeCommandData(cmds) {
    return cmds.map((c) => {
...
export const registrar = token && appId ? new Registrar(new REST({ version: "10" }).setToken(token), appId) : null;
if (!registrar) {
    console.warn("[registrar] Missing DISCORD_TOKEN/BOT_TOKEN or DISCORD_APP_ID/APP_ID; " +
