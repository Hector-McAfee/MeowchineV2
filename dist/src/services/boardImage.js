...
function registerFont(p, family) {
    try {
...
export function tileComplete(state, team, idx) {
...
export function tilePending(state, team, idx) {
    return Object.values(state.pending).some(p => p.kind === "drop" && p.team === team && p.tile === idx);
}
function dropSummary(state, team, idx) {
    const cfg = state.tiles[idx];
    if (!cfg || !cfg.drops)
        return [];
    const prog = state.progress[team]?.[idx] ?? {};
...
export async function renderBoardImage(state, team) {
    ...
    for (let r = 0; r < side; r++) {
        for (let c = 0; c < side; c++) {
            ...
            if (aliasEntry && aliasEntry[1])
                bossLabel = aliasEntry[1];
            ...
            if (lines.length > 3)
                ctx.fillText(`+${lines.length - 3} more...`, x + TILE / 2, y + TILE - 22);
            ctx.textAlign = "left";
