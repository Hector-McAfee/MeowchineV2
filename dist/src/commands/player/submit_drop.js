...
export async function execute(i) {
    ...
    if (!s.started)
        return i.reply({ embeds: [embed("Bingo Hasn't Started", "Admins must run `/start_bingo` to begin accepting drop submissions.")], ephemeral: true });
    ...
    if (s.options.chestVerify) {
        ...
        if (missing.length > 0) {
            return i.reply({ embeds: [embed("Chest Verification Required", "You must verify these before submitting drops:\n" + missing.map(m => `• ${m}`).join("\n"), COLORS.warn)], ephemeral: true });
        }
    }
    ...
    if (!outCh)
        return i.reply({ embeds: [embed("No Output Channel", "Ask an admin to **/set_output** here.")], ephemeral: true });
