...
export const add_boss = new SlashCommandBuilder()
    ...
    .addIntegerOption(o => o.setName("tile").setDescription("Tile number").setRequired(true).setAutocomplete(true))
    .addStringOption(o => o.setName("boss").setDescription("Boss name").setRequired(true).setAutocomplete(true))
    .addBooleanOption(o => o.setName("or").setDescription("If true, any one drop can complete the tile"))
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild);
export const add_drop = new SlashCommandBuilder()
    .setName("add_drop")
    .setDescription("Add a drop target to a boss’s tile (admin)")
    .addStringOption(o => o.setName("boss").setDescription("Boss (already set on a tile)").setRequired(true).setAutocomplete(true))
    .addStringOption(o => o.setName("drop").setDescription("Drop label (from presets)").setRequired(true).setAutocomplete(true))
    .addIntegerOption(o => o.setName("amount").setDescription("Target amount for this drop").setRequired(true))
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild);
...
export const remove_drop = new SlashCommandBuilder()
    ...
    .addIntegerOption(o => o.setName("tile").setDescription("Tile number").setRequired(true).setAutocomplete(true))
    .addStringOption(o => o.setName("drop").setDescription("Drop label").setRequired(true).setAutocomplete(true))
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild);
export const set_teams_drop = new SlashCommandBuilder()
    ...
    .addIntegerOption(o => o.setName("team").setDescription("Team number").setRequired(true).setAutocomplete(true))
    .addStringOption(o => o.setName("boss").setDescription("Boss (as shown on the board)").setRequired(true).setAutocomplete(true))
    .addStringOption(o => o.setName("drop").setDescription("Drop label").setRequired(true).setAutocomplete(true))
    .addIntegerOption(o => o.setName("amount").setDescription("Verified amount").setRequired(true))
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild);
/** ---------------------- Execs ---------------------- */
export async function exec_add_boss(i) {
    if (!isAdmin(i.member)) {
    ...
    await i.reply({
        embeds: [
            embed("Boss Set", `Tile **${tile}** → **${bossInput}**\nMode: **${orMode ? "OR" : "AND"}**`),
        ],
    });
}
export async function exec_add_drop(i) {
    if (!isAdmin(i.member)) {
        return i.reply({ embeds: [embed("Admin Only", "No permission.")], flags: 64 });
    }
    const bossInput = i.options.getString("boss", true);
    ...
    let tileNum = null;
    for (const [tileStr, data] of Object.entries(s.tiles ?? {})) {
...
export async function exec_remove_boss(i) {
    if (!isAdmin(i.member)) {
        return i.reply({ embeds: [embed("Admin Only", "No permission.")], flags: 64 });
    }
    const tile = i.options.getInteger("tile", true);
    const s = await load(i.guildId);
    delete s.tiles[tile];
    for (const team of Object.keys(s.progress))
        delete s.progress[team]?.[tile];
    for (const [mid, p] of Object.entries(s.pending)) {
        if (p.kind === "drop" && p.tile === tile)
            delete s.pending[mid];
    }
    await save(i.guildId, s);
    await i.reply({ embeds: [embed("Tile Cleared", `Tile **${tile}** cleared.`)] });
}
export async function exec_remove_drop(i) {
    if (!isAdmin(i.member)) {
        return i.reply({ embeds: [embed("Admin Only", "No permission.")], flags: 64 });
    }
    const tile = i.options.getInteger("tile", true);
    ...
    for (const team of Object.keys(s.progress))
        delete s.progress[team]?.[tile]?.[drop];
    for (const [mid, p] of Object.entries(s.pending)) {
        if (p.kind === "drop" && p.tile === tile && p.drop === drop)
            delete s.pending[mid];
    }
    await save(i.guildId, s);
    await i.reply({ embeds: [embed("Drop Removed", `Removed **${drop}** from Tile **${tile}**.`)] });
}
export async function exec_set_teams_drop(i) {
    if (!isAdmin(i.member)) {
        return i.reply({ embeds: [embed("Admin Only", "No permission.")], flags: 64 });
    }
