...
export const set_input = new SlashCommandBuilder()
    ...
    .setDefaultMemberPermissions(PermissionFlagsBits.ManageGuild);
export async function exec_set_output(i) {
    if (!isAdmin(i.member))
        return i.reply({ embeds: [embed("Admin Only", "No permission.")], ephemeral: true });
...
export async function exec_set_announcments(i) {
    if (!isAdmin(i.member))
        return i.reply({ embeds: [embed("Admin Only", "No permission.")], ephemeral: true });
    const s = await load(i.guildId);
...
export async function exec_set_input(i) {
    if (!isAdmin(i.member))
        return i.reply({ embeds: [embed("Admin Only", "No permission.")], ephemeral: true });
