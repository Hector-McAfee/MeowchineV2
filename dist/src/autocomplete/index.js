    ...
    try {
        ...
        if (name === "add_boss") {
            if (focused.name === "tile")
                return i.respond(await suggestEmptyTileNumbers(i));
            if (focused.name === "boss")
                return i.respond(await suggestAllCatalogBosses(i, BOSS_CATALOG));
        }
        if (name === "add_drop") {
            if (focused.name === "boss")
                return i.respond(await suggestBossesForSetup(i));
            if (focused.name === "drop")
                return i.respond(await suggestDropsForBoss(i, BOSS_CATALOG));
            if (focused.name === "amount")
                return i.respond([{ name: "1", value: 1 }, { name: "2", value: 2 }, { name: "3", value: 3 }]);
        }
        if (name === "set_teams_drop") {
            if (focused.name === "boss")
                return i.respond(await suggestUsedBosses(i));
            if (focused.name === "drop")
                return i.respond(await suggestSubmitDropsForBoss(i));
        }
        if (name === "remove_boss") {
            if (focused.name === "tile")
                return i.respond(await suggestTileNumbers(i));
        }
        if (name === "remove_drop") {
            if (focused.name === "tile")
                return i.respond(await suggestTileNumbers(i));
