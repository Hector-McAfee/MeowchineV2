...
export function wireReactionHandler(client) {
    client.on("messageReactionAdd", async (reaction, user) => {
        try {
            ...
            if (!pending)
                return;
            const outCh = reaction.message.channel;
            const base = EmbedBuilder.from(reaction.message.embeds[0] ?? new EmbedBuilder());
            if (emoji === "✅")
                base.setColor(COLORS.ok).setDescription((base.data.description ?? "").replace(/\(pending\)/i, `**VERIFIED by ${user.username}**`));
            else
                base.setColor(COLORS.err).setDescription(`${base.data.description ?? ""}\n**DENIED by ${user.username}**`);
            await reaction.message.edit({ embeds: [base] }).catch(() => { });
            const teamCh = reaction.message.guild.channels.cache.get(pending.submitChannelId);
            delete state.pending[reaction.message.id];
            if (pending.kind === "drop") {
                const { team, tile, drop, target, delta, snapshot } = pending;
                    ...
                    if (emoji === "✅") {
                        ...
                        if (newVal >= target) {
                            for (const [mid, p] of Object.entries(fresh.pending)) {
                                if (p.kind === "drop" && p.team === team && p.tile === tile && p.drop === drop) {
                        ...
                        if (fresh.options.trackFirstLine && !fresh.firstLineAwardedTo) {
                            if (checkAnyLineComplete(fresh, team)) {
                                fresh.firstLineAwardedTo = team;
                                if (fresh.announcementsChannelId) {
                                    const ann = reaction.message.guild.channels.cache.get(fresh.announcementsChannelId);
                                    await ann?.send({ embeds: [embed("First Line Complete! 🟩", `Team **${team}** is first to complete a line!`, COLORS.ok)] });
                                }
                            }
                        }
                        await onBoardCompleteMaybeAnnounce(reaction.message.guild, fresh, team);
                    }
                    else {
                        if (pending.teamMessageId && teamCh) {
                            const tm = await teamCh.messages.fetch(pending.teamMessageId).catch(() => null);
                ...
                const { userId, boss, team } = pending;
                if (emoji === "✅") {
                    const set = new Set(state.chestVerifiedUsers[userId] ?? []);
                    set.add(boss);
                    state.chestVerifiedUsers[userId] = Array.from(set).sort();
                }
                if (pending.teamMessageId && reaction.message.guild.channels.cache.get(pending.submitChannelId)) {
                    const teamCh2 = reaction.message.guild.channels.cache.get(pending.submitChannelId);
